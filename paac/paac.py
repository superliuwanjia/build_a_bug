...
class PAACLearner(ActorLearner):
    ...
    def choose_next_actions(network, num_actions, states, session, is_viz = False, counter = 0):
        network_output_v, network_output_pi, network_output_retina = session.run(
        # network_output_v, network_output_pi, network_output_retina = session.run(
            [network.output_layer_v,
             network.output_layer_pi,
             network.output_retina],
            feed_dict={network.input_ph: states})
        # print(states.shape, network_output_retina.shape, network_output_pi.shape)
        # print(jh)
        # if is_viz:
            # print("counter detected gettinh  the ")
            # PAACLearner.__visualise(states, network_output_retina, counter)
    ...
    def __visualise(states, network_output_retina, counter = 1, name = "viz"):
        dim = (32, 32)
        print("dims",dim)
