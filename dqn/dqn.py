...
class DeepQNetwork:
    ...
    def tf_retina(self, video, alpha=0.5, mu_on=0.00, mu_off=-0.1):
        ...
        # decay constant must be smaller than 1
        # assert alpha <= 1 and alpha >= 0, 'decay constant must be positive and less than 1'
        video_shape = video.get_shape().as_list()
        video = tf.to_float(video) + 1e-3
        print(video.get_shape().as_list())
        assert video_shape[0] == 4, 'time not first dim'
        split_vids = tf.split(video, num_or_size_splits=video_shape[0], axis=0)
        emas = [split_vids[0]]
        for t in range(1, video_shape[0]):
            emas.append((1 - alpha) * emas[t - 1] + alpha * split_vids[t])
        print([temp.get_shape().as_list() for temp in emas])
        ema = tf.concat(emas, axis=0)
        print(ema.get_shape().as_list())
        # compute relative change
    ...
    def buildNetwork(self, name, trainable, numActions):
        ...
        dqn_in = self.tf_apply_preprocess(x)
    ...
    def train(self, batch, stepNumber):
        ...
        _, sum_str, proc_x = self.sess.run([self.train_step, self.merged, self.event], feed_dict={
                                                                            self.x: x,
                                                                            self.a: a,
                                                                            self.y_: y_
                                                                        })
