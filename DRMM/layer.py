...
class Layer():
	...
	def _initialize(self):
		...
		tf.summary.histogram("prun_mat", self.prun_mat)
		...
		tf.summary.histogram("prun_synap_mat", self.prun_synap_mat)
		...
		tf.summary.histogram("lambdas_t", self.lambdas_t)
	...
	def get_important_latents_BU(self, input, betas):
		...
		if self.border_mode != 'FULL':
			...
			latents_before_BN = tf.transpose(latents_before_BN, [0, 3, 1, 2])
		# tf.image_summary("betas", tf.transpose(betas,[2,3,1,0]))
